...
async def settings(ctx):
    ...
    embed = discord.Embed(title="Guild Settings", color=0x3498db)
...
class Dropdown(discord.ui.Select):
    def __init__(self, interaction: discord.Interaction):
        ...
        self.user_id = interaction.user.id  # Store user ID to track them
        options = [
            discord.SelectOption(label="Option 1", description="Set log channel"),
            discord.SelectOption(label="Option 2", description="Sends 'owo'"),
        ]
    ...
    async def callback(self, interaction: discord.Interaction):
        if self.values[0] == "Option 1":
            await interaction.response.send_message(
                "Please mention the log channel (e.g., `#channel-name`). You have **60 seconds**.", ephemeral=True
            )

            # Store interaction message & start timeout task
            bot.waiting_for_channel[interaction.user.id] = {
                "message": interaction.message,
                "task": asyncio.create_task(self.wait_for_channel_selection(interaction.user.id))
            }
        else:
            await interaction.response.send_message("owo", ephemeral=True)

    async def wait_for_channel_selection(self, user_id):
        """Timeout function to clear waiting state if user takes too long."""
        await asyncio.sleep(60)  # Wait for 60 seconds
        if user_id in bot.waiting_for_channel:
            dropdown_data = bot.waiting_for_channel.pop(user_id)
            dropdown_message = dropdown_data["message"]

            # Edit the original dropdown message to indicate timeout
            await dropdown_message.edit(content="❌ Log channel selection **timed out**. Please try again.")
...
# Dictionary to track users waiting for a channel selection
bot.waiting_for_channel = {}
...
async def on_message(message):
    ...
    if message.author.id in bot.waiting_for_channel:
        dropdown_data = bot.waiting_for_channel.pop(message.author.id)
        dropdown_message = dropdown_data["message"]
        timeout_task = dropdown_data["task"]
        ...
        if message.content.startswith("<#") and message.content.endswith(">"):
            try:
                ...
                if channel:
                    print(f"[DEBUG] Extracted Channel ID: {channel_id} ({channel.name})")  # Debugging
                    update_guild_settings(message.guild.id, log_channel_id=channel.id)
                    await message.delete()
                    await dropdown_message.edit(content=f"✅ Log channel set to {channel.mention}")
                else:
                    await dropdown_message.edit(content="❌ Invalid channel. Please try again.")

            except ValueError:
                await dropdown_message.edit(content="❌ Invalid input. Please mention a channel.")
            ...
            try:
                ...
                if owner_label:
                    ...
                    if owner_name_tag:
                        ...
                        cursor.execute("SELECT * FROM trackables WHERE code = ?", (code,))
...
# TB PURGE
    ...
    async def purge(self, interaction: discord.Interaction, username: str):
        ...
        try:
            cursor.execute("SELECT COUNT(*) FROM trackables WHERE gc_username = ?", (username,))
            count = cursor.fetchone()[0]

            if count > 0:
                cursor.execute("DELETE FROM trackables WHERE gc_username = ?", (username,))
                conn.commit()
        ...
        try:
            ...
            if existing_entry:
                ...
                if gc_username == interaction.user.display_name:
                    cursor.execute("DELETE FROM trackables WHERE code = ?", (code,))
                    conn.commit()
    ...
    async def forceremove(self, interaction: discord.Interaction, code: str):
        ...
        try:
            ...
            if existing_entry:
                cursor.execute("DELETE FROM trackables WHERE code = ?", (code,))
                conn.commit()
                await interaction.response.send_message(f"The TB with code `{code}` has been forcible removed from the database.")
                ...
                await interaction.response.send_message(f"No TB with the code `{code}` was found in the database.")
                await bot.get_channel(1322311043493531669).send(f"{interaction.user.mention} ({interaction.user.name}) tried to forcible remove TB `{code}` from the database but it wasn't there.")
        except Exception as e:
            await interaction.response.send_message(f"An error occurred while trying to remove the TB. Please ask an Administrator to check the logs.")
...
# TB VIEW
    ...
    async def view(self, interaction: discord.Interaction):
        ...
        cursor.execute("SELECT * FROM trackables")
        trackables = cursor.fetchall()
...
# TB BULKVIEW
    ...
    async def bulkview(self, interaction: discord.Interaction):
        ...
        try:
            cursor.execute("SELECT code FROM trackables")
            trackables = cursor.fetchall()
