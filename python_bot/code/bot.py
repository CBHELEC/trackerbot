...
async def on_ready():
    ...
    await bot.change_presence(
        activity=discord.CustomActivity(f"Existing in {len(bot.guilds)} Servers"))
...
async def update_presence():
    ...
    if current_server_count != last_server_count:
        await bot.change_presence(
            activity=discord.CustomActivity(f"Existing in {current_server_count} Servers"))
...
async def main():
    config = uvicorn.Config(dbl, host="0.0.0.0", port=8080, log_level="warning")
    server = uvicorn.Server(config)

    config2 = uvicorn.Config(ping_app, host="0.0.0.0", port=9090, log_level="warning")
    server2 = uvicorn.Server(config2)

    config3 = uvicorn.Config(topgg, host="0.0.0.0", port=9797, log_level="warning")
    ...
    try:
        await asyncio.gather(dbl_task, ping_task, topgg_task, bot_task)
    except asyncio.CancelledError:
        print("Shutting Down...")
        for task in [dbl_task, ping_task, topgg_task, bot_task]:
            task.cancel()
        await asyncio.gather(dbl_task, ping_task, topgg_task, bot_task, return_exceptions=True)
    finally:
        if not bot.is_closed():
            await bot.close()

        for server_instance in [server, server2, server3]:
            if server_instance.started:
                await server_instance.shutdown()
                await asyncio.sleep(0.05)

        for app in [dbl, ping_app, topgg]:
            if hasattr(app.state, "session") and not app.state.session.closed:
